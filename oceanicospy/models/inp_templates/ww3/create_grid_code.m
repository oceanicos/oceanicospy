...
% 0.a Path to directories 
  ...
  bin_dir = 'bin_path';             % matlab scripts location
  ref_dir = 'reference_path';  % reference data location
  out_dir = 'output_path';                  % output directory
...
% 0.b Design grid parameters
  ...
  fname = 'name_case';              % file name prefix 
  ...
  dx = res_x;
  dy = res_y; 
  lon1d = [lonmin:dx:lonmax];
  lat1d = [latmin:dy:latmax]; 
...
% 5. Output to ascii files for WAVEWATCH III
  ...
  write_ww3file([out_dir,'/bottom.inp'],d);                 

  write_ww3file([out_dir,'/mask.inp'],m4);             

  d1 = round((sx1)*obstr_scale);
  d2 = round((sy1)*obstr_scale);
  write_ww3obstr([out_dir,'/obstr.inp'],d1,d2);            
